...
export function VideoProcessingAdminPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [nextJobs, nextHealth, episodePage, movieItems] = await Promise.all([api<ProcessingRow[]>('/admin/video-processing'), api<WorkerHealth>('/admin/video-processing/worker-health'), api<EpisodePage>('/admin/episodes?page=1&limit=100'), api<Movie[]>('/admin/movies')]);
        if (mounted) { setJobs(nextJobs); setHealth(nextHealth); setEpisodes(episodePage.items); setMovies(movieItems); setError(null); }
...
function formatStage(stage?: string) { if (!stage) return 'Pendiente'; if (stage.startsWith('GENERATING_HLS_')) return `Generando ${stage.replace('GENERATING_HLS_', '').replace('P', 'p')}`; return ({ QUEUED: 'En cola', PROBING: 'Analizando video', PREPARING: 'Preparando conversion', EXTRACTING_SUBTITLES: 'Extrayendo subtitulos', GENERATING_THUMBNAIL: 'Generando miniatura', VALIDATING: 'Validando salida', UPLOADING_OUTPUT: 'Guardando HLS', AWAITING_ASSOCIATION: 'Pendiente de asociar', ASSOCIATING: 'Asociando', COMPLETED: 'Completado', FAILED: 'Fallido', CANCELLED: 'Cancelado' } as Record<string, string>)[stage] ?? stage; }
