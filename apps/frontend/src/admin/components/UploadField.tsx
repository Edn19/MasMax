...
import { cancelResumableUpload, getResumableUpload, initiateResumableUpload, listResumableUploads, matchesResumableFile, ResumableUploadResult, ResumableUploadSession, UploadControl, uploadResumableVideo, VideoProcessingJob, VideoProcessingMode } from '../../lib/resumable-upload';
...
export type UploadedDetails = { mediaFileId?: string; originalUrl?: string; thumbnailUrl?: string; processingJobId?: string; directlyPlayable?: boolean; processingMode?: VideoProcessingMode; compatibilityMessage?: string; width?: number; height?: number; durationSec?: number; videoCodec?: string; audioCodec?: string; fastStart?: boolean | null };
...
export function UploadField({ type, label, onUploaded, onProcessingJob, selectedProcessingJobId, target }: UploadFieldProps) {
  ...
  const [processingMode, setProcessingMode] = useState<VideoProcessingMode>('ORIGINAL');
  ...
  useEffect(() => {
    ...
    const completedUrl = processingJob.kind === 'REMUX' ? processingJob.outputUrl : processingJob.masterUrl;
    if (processingJob.status === 'COMPLETED' && completedUrl) {
      if (completedJobsRef.current.has(processingJob.id)) return;
      completedJobsRef.current.add(processingJob.id);
      setProcessingJobId(null);
      const compatible = uploadedResult ? directlyPlayableOriginal(uploadedResult) : false;
      onUploadedRef.current(completedUrl, processingJob.kind === 'REMUX' ? 'video/mp4' : 'application/vnd.apple.mpegurl', { mediaFileId: uploadedResult?.mediaId, originalUrl: uploadedResult?.url, thumbnailUrl: processingJob.thumbnailUrl ?? undefined, processingJobId: processingJob.id, directlyPlayable: processingJob.kind === 'REMUX' || compatible, processingMode: processingJob.kind, compatibilityMessage: processingJob.kind === 'REMUX' ? 'MP4 remux compatible para reproduccion nativa.' : compatibilityMessage(uploadedResult, compatible), width: uploadedResult?.metadata?.width, height: uploadedResult?.metadata?.height, durationSec: uploadedResult?.metadata?.durationSec, videoCodec: uploadedResult?.metadata?.videoCodec, audioCodec: processingJob.kind === 'REMUX' ? 'aac' : uploadedResult?.metadata?.audioTracks?.[0]?.codec, fastStart: processingJob.kind === 'REMUX' ? true : uploadedResult?.metadata?.fastStart });
      toast.success(processingJob.kind === 'REMUX' ? 'MP4 remux listo' : 'Video HLS procesado y listo');
    }
  ...
  async function startVideo(selected: File, current: ResumableUploadSession) {
    ...
    try {
      ...
      if (outcome.result) {
        ...
        if (outcome.result.processingJob) { const configuredJob = await patchJson<VideoProcessingJob>(`/admin/video-processing/${outcome.result.processingJob.id}/settings`, target ? { retainOriginal: true, targetType: target.type, targetId: target.id } : { retainOriginal: true }); setUploadedResult(outcome.result); setProcessingJobId(configuredJob.id); processing.track(configuredJob); onProcessingJob?.(configuredJob); toast.success('Video subido. Ya puedes guardar mientras se procesa'); }
        else { const compatible = directlyPlayableOriginal(outcome.result); onUploaded(outcome.result.url, outcome.result.mimeType, { mediaFileId: outcome.result.mediaId, originalUrl: outcome.result.url, directlyPlayable: compatible, processingMode: 'ORIGINAL', compatibilityMessage: compatibilityMessage(outcome.result, compatible), width: outcome.result.metadata?.width, height: outcome.result.metadata?.height, durationSec: outcome.result.metadata?.durationSec, videoCodec: outcome.result.metadata?.videoCodec, audioCodec: outcome.result.metadata?.audioTracks?.[0]?.codec, fastStart: outcome.result.metadata?.fastStart }); setCompleted(true); toast.success('Original almacenado sin transcodificar'); }
      }
    } catch (error) {
  ...
  return (
    <div className="min-w-0 text-sm text-slate-300">
      ...
      <div className={`rounded-xl border border-dashed p-4 text-center transition ${dragging ? 'border-brand bg-brand/10' : error ? 'border-coral/70 bg-coral/5' : 'border-line bg-ink/30 hover:border-slate-600'}`} onDragEnter={(event) => { event.preventDefault(); setDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setDragging(false); }} onDrop={drop}>
        ...
        {!file ? <><span className="mx-auto grid h-11 w-11 place-items-center rounded-xl bg-brand/10 text-brand">{type === 'image' ? <ImageIcon size={22} /> : <UploadCloud size={22} />}</span><p className="mt-3 font-medium text-slate-200">Arrastra {type === 'image' ? 'una imagen' : 'un video MP4 o MKV'} aqui</p><p className="mt-1 text-xs text-slate-400">o selecciona un archivo desde tu equipo</p><button type="button" className="button-secondary mt-3" disabled={uploading} onClick={() => inputRef.current?.click()}><FileUp size={16} />Seleccionar archivo</button><p className="mt-3 text-xs text-slate-500">{type === 'video' ? `MP4 o MKV hasta ${maxVideoUploadMb} MB. Elige conservar el original, crear MP4 compatible o generar HLS.` : `JPG, PNG o WebP · maximo ${Number(import.meta.env.VITE_MAX_IMAGE_UPLOAD_MB ?? 10)} MB`}</p></> : <div className="flex flex-col items-center gap-3 sm:flex-row sm:text-left"><span className={`grid h-12 w-12 shrink-0 place-items-center rounded-xl ${completed ? 'bg-mint/10 text-mint' : 'bg-brand/10 text-brand'}`}>{completed ? <CheckCircle2 size={23} /> : type === 'image' ? <ImageIcon size={23} /> : <FileUp size={23} />}</span><div className="min-w-0 flex-1"><p className="truncate font-medium text-slate-200">{file.name}</p><p className="mt-1 text-xs text-slate-400">{formatBytes(String(file.size))} · {file.type || 'Tipo no informado'}</p>{completed && <p className="mt-1 text-xs font-medium text-mint">Carga completada</p>}</div><div className="flex gap-2"><button type="button" className="button-ghost" disabled={uploading} onClick={() => inputRef.current?.click()} aria-label="Reemplazar archivo"><RotateCcw size={16} />Reemplazar</button><button type="button" className="icon-button text-coral" disabled={uploading} onClick={clearSelection} aria-label="Eliminar seleccion"><Trash2 size={17} /></button></div></div>}
      </div>
      {error && <p role="alert" className="mt-1.5 text-xs font-medium text-coral">{error}</p>}
      {type === 'video' && !session && !processingJob && <fieldset className="mt-3 rounded-lg border border-line p-3 text-left"><legend className="px-1 text-xs font-semibold text-slate-200">Procesamiento del video</legend><label className="flex items-start gap-2 text-xs text-slate-300"><input type="radio" name={`${label}-processing-mode`} value="ORIGINAL" checked={processingMode === 'ORIGINAL'} onChange={() => setProcessingMode('ORIGINAL')} /><span><strong className="block text-slate-200">Mantener archivo original</strong>No ejecuta FFmpeg ni genera copias.</span></label><label className="mt-3 flex items-start gap-2 text-xs text-slate-300"><input type="radio" name={`${label}-processing-mode`} value="REMUX" checked={processingMode === 'REMUX'} onChange={() => setProcessingMode('REMUX')} /><span><strong className="block text-slate-200">Crear MP4 rapido (remux)</strong>Copia el video H.264; convierte solo audio incompatible a AAC.</span></label><label className="mt-3 flex items-start gap-2 text-xs text-slate-300"><input type="radio" name={`${label}-processing-mode`} value="HLS" checked={processingMode === 'HLS'} onChange={() => setProcessingMode('HLS')} /><span><strong className="block text-slate-200">Transcodificar a HLS</strong>Genera perfiles adaptativos y conserva el original.</span></label></fieldset>}
...
function directlyPlayableOriginal(result: ResumableUploadResult) { const video = result.metadata?.videoCodec?.toLowerCase() ?? ''; const audio = result.metadata?.audioTracks?.[0]?.codec?.toLowerCase() ?? ''; return result.mimeType === 'video/mp4' && (video === 'h264' || video === 'avc1') && (!audio || audio === 'aac' || audio === 'mp3'); }
function compatibilityMessage(result: ResumableUploadResult | null | undefined, compatible: boolean) { if (!compatible) return 'El archivo fue guardado, pero puede requerir transcodificacion para reproducirse en todos los navegadores.'; if (result?.metadata?.fastStart === false) return 'Compatible para reproduccion directa; el MP4 no tiene inicio rapido y puede tardar mas en comenzar.'; return 'Compatible para reproduccion directa.'; }
