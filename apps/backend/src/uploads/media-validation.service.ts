...
export class MediaValidationService {
  ...
  async validateVideo(file: Express.Multer.File) {
    try {
      ...
      await this.storage.putFile(relativePath, file.path, { contentType: 'video/mp4' });
      try {
        const media = await this.prisma.mediaFile.create({ data: { originalName: file.originalname, storageName, relativePath, mimeType: 'video/mp4', extension: '.mp4', sizeBytes: BigInt(file.size), checksum, mediaType: MediaType.VIDEO, status: MediaStatus.READY, width: video.width, height: video.height, durationSec: Number(probe.format?.duration ?? 0), bitrate: Number(probe.format?.bit_rate ?? 0) || null, videoCodec: video.codec_name, audioCodec: probe.streams?.find((stream) => stream.codec_type === 'audio')?.codec_name } });
        return { url: this.storage.publicUrl(relativePath), filename: storageName, originalName: file.originalname, size: file.size, mimeType: 'video/mp4', mediaId: media.id, metadata: { width: media.width, height: media.height, durationSec: media.durationSec, videoCodec: media.videoCodec } };
