...
export class MediaService {
  ...
  async authorize(userId: string, sessionId: string, dto: AuthorizeMediaDto, ip?: string) {
    ...
    const resource = dto.episodeId
      ? await this.prisma.episode.findFirst({ where: publishedEpisodeWhere({ id: dto.episodeId }), select: { videoUrl: true, processedVideoUrl: true, originalVideoUrl: true } })
    ...
    if (hls) {
      ...
      return { url: `/api/media/hls?token=${encodeURIComponent(token)}&path=master.m3u8`, protected: true, expiresAt: new Date(expires * 1000).toISOString(), originalUrl: resource.originalVideoUrl };
    }
    if (!path || !/^videos\/[a-zA-Z0-9._-]+\.mp4$/.test(path)) return { url: mediaUrl, protected: false, originalUrl: resource.originalVideoUrl };
    if (this.storage.driverName === 's3') {
      if (!await this.storage.exists(path)) throw new NotFoundException('El archivo multimedia no esta disponible');
      await this.prisma.viewLog.create({ data: { userId, sessionId, episodeId: dto.episodeId || null, movieId: dto.movieId || null, ip } });
      return { url: await this.storage.temporaryUrl(path, expires - Math.floor(Date.now() / 1000)), protected: true, expiresAt: new Date(expires * 1000).toISOString(), originalUrl: resource.originalVideoUrl };
    }
    ...
    return { url: `/api/media/stream?${query}`, protected: true, expiresAt: new Date(expires * 1000).toISOString(), originalUrl: resource.originalVideoUrl };
  }
