...
export class VideoProcessingController {
  ...
  @Get('jobs/available') availableJobs(@CurrentUser() user: JwtUser) { return this.processing.listAvailable(user.sub); }
  ...
  @Post(':id/associate') associate(@CurrentUser() user: JwtUser, @Param('id') id: string, @Body() dto: AssociateVideoProcessingDto) { return this.processing.associate(id, dto, user.sub); }
  @Patch(':id/settings') configure(@CurrentUser() user: JwtUser, @Param('id') id: string, @Body() dto: ConfigureVideoProcessingDto) { return this.processing.configure(id, dto, user.sub); }
