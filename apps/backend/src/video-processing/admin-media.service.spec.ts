...
const completedJob = {
  id: 'job123', status: 'COMPLETED', processingStage: 'COMPLETED', progress: 100, masterPath: 'hls/job123/master.m3u8', thumbnailPath: null,
...
function setup(job: unknown = completedJob) {
  ...
  const prisma = {
    ...
    mediaFile: { update: vi.fn().mockResolvedValue({}) }, episode: { update: vi.fn().mockResolvedValue({}), updateMany: vi.fn() }, movie: { update: vi.fn(), updateMany: vi.fn() },
...
describe('AdminMediaService destructive rules', () => {
  ...
  it('deletes only the HLS prefix, unpublishes content, and preserves the original', async () => {
    ...
    expect(tx.episode.updateMany).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ published: false }) }));
  });
...
describe('AdminMediaService listing', () => {
  it('normalizes processing content, active uploads, and unassigned files', async () => {
    ...
    const storage = { publicUrl: vi.fn((key: string) => `/uploads/${key}`) };
